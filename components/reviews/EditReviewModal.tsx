"use client";

import { useActionState, useEffect } from "react";
import { Modal } from "@/components/dashboard/Modal";
import { updateReviewAction } from "@/lib/actions/reviews";
import type { ActionState } from "@/lib/actions/rooms";
import { REVIEW_SOURCES } from "@/lib/constants";

const initialState: ActionState = {};

const inputClasses =
  "w-full rounded-xl border border-[#e1e0d9] bg-white px-3.5 py-2.5 text-sm text-[#0b0b0b] focus:outline-none focus:ring-2 focus:ring-[#2a78d6]/30";

type RoomOption = { id: string; number: string; name: string };

export type EditableReview = {
  id: string;
  guestName: string;
  guestEmail: string;
  roomId: string;
  rating: number;
  source: string;
  reviewText: string;
};

export function EditReviewModal({
  open,
  onClose,
  review,
  rooms,
}: {
  open: boolean;
  onClose: () => void;
  review: EditableReview;
  rooms: RoomOption[];
}) {
  const action = updateReviewAction.bind(null, review.id);
  const [state, formAction, pending] = useActionState(action, initialState);

  // `onClose` belongs to the parent row, not this component, so closing on
  // success has to happen in an effect rather than during render.
  useEffect(() => {
    if (state.success) {
      onClose();
    }
  }, [state.success, onClose]);

  return (
    <Modal open={open} onClose={onClose} title="Edit Review">
      <form action={formAction} className="flex flex-col gap-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="mb-1.5 block text-xs font-medium text-[#52514e]">Guest Name</label>
            <input name="guestName" type="text" required defaultValue={review.guestName} className={inputClasses} />
          </div>
          <div>
            <label className="mb-1.5 block text-xs font-medium text-[#52514e]">Guest Email</label>
            <input name="guestEmail" type="email" required defaultValue={review.guestEmail} className={inputClasses} />
          </div>
        </div>

        <div className="grid grid-cols-3 gap-4">
          <div className="col-span-2">
            <label className="mb-1.5 block text-xs font-medium text-[#52514e]">Room</label>
            <select name="roomId" required defaultValue={review.roomId} className={inputClasses}>
              {rooms.map((room) => (
                <option key={room.id} value={room.id}>
                  {room.number} — {room.name}
                </option>
              ))}
            </select>
          </div>
          <div>
            <label className="mb-1.5 block text-xs font-medium text-[#52514e]">Rating</label>
            <select name="rating" defaultValue={review.rating} className={inputClasses}>
              {[5, 4, 3, 2, 1].map((n) => (
                <option key={n} value={n}>
                  {n} Star{n === 1 ? "" : "s"}
                </option>
              ))}
            </select>
          </div>
        </div>

        <div>
          <label className="mb-1.5 block text-xs font-medium text-[#52514e]">Source</label>
          <select name="source" defaultValue={review.source} className={inputClasses}>
            {REVIEW_SOURCES.map((source) => (
              <option key={source} value={source}>
                {source}
              </option>
            ))}
          </select>
        </div>

        <div>
          <label className="mb-1.5 block text-xs font-medium text-[#52514e]">Review Text</label>
          <textarea
            name="reviewText"
            rows={3}
            required
            defaultValue={review.reviewText}
            className={`${inputClasses} resize-none`}
          />
        </div>

        {state.error && (
          <p className="rounded-lg bg-[#d03b3b]/10 px-3 py-2 text-sm text-[#d03b3b]">{state.error}</p>
        )}

        <button
          type="submit"
          disabled={pending}
          className="mt-1 w-full rounded-xl bg-[#2a78d6] py-2.5 text-sm font-semibold text-white hover:bg-[#1c5cab] disabled:opacity-70"
        >
          {pending ? "Saving..." : "Save Changes"}
        </button>
      </form>
    </Modal>
  );
}
