...
export async function updateStaffAction(id: string, _prevState: ActionState, formData: FormData): Promise<ActionState> {
  await requireSession();
  ...
  await prisma.staff.update({
    where: { id },
    data: { name, role, department, status, phone, email },
  });
